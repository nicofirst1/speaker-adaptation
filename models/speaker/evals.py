...
def eval_beam_histatt(
    ...
    refs_file="speaker_outputs/refs_" + file_name + ".json"
    ...
    logger.log_artifact(refs_file, "refs", artifact_type="json", epoch=None, description=f"References output for spekaer on {split} split")
    #
    ...
    hyps_file="speaker_outputs/hyps_" + file_name + ".json"
    with open(hyps_file, "w") as f:
        json.dump(hypotheses, f)

    logger.log_artifact(hyps_file, "hyps", artifact_type="json", epoch=None, description=f"Hypotesis output for spekaer on {split} split")
