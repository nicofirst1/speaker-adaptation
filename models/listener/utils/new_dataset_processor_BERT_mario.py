...
def process4bert(data, path, split, model, tokenizer):

    # chain dataset - lengths are going to be different
    chain_dataset = []
    chains_path = path + split + '_BERTNEW_chains.json'

    chain_count = 0

    utterance_dataset = defaultdict()
    utterances_path = path + split + '_BERTNEW_utterances.pickle'

    utterance_count = 0

    representation_dataset = defaultdict()
    representations_path = path + split + '_BERTNEW_representations.pickle'  # for torch tensors
...
if __name__ == '__main__':
    ...
    for domain in DOMAINS:
        ...
        domain_path = '../data/chains/{}/'.format(domain)
        ...
        process4bert(train, domain_path, 'train', model, tokenizer)

        print('processing val...')
        process4bert(val, domain_path, 'val', model, tokenizer)

        print('processing test...')
        process4bert(test, domain_path, 'test', model, tokenizer)

        print('processing test...')
        process4bert(test_seen, domain_path, 'test_seen', model, tokenizer)

        print('processing test...')
        process4bert(test_unseen, domain_path, 'test_unseen', model, tokenizer)
