...
class SimulatorModel(ListenerModel):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, img_dim, att_dim, dropout_prob):
        super(ListenerModel).__init__(vocab_size, embedding_dim, hidden_dim, img_dim, att_dim, dropout_prob)
        self.speaker_influence=nn.Linear(6, self.hidden_dim)

    def forward(self, text: torch.Tensor, separate_images: torch.Tensor, visual_context: torch.Tensor, prev_hist: List,
