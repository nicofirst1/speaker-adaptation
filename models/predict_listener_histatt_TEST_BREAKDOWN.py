...
def evaluate_trained_model(split_data_loader, split_dataset, model, device, hyps, model_bert,
    ...
    for ii, data in enumerate(split_data_loader):
        ...
        print(image_set, target, target_img)
        ...
        if ii in ids_first:
            ...
            accuracies_later.append(float(correct))
        ...
        if out.shape[0] > 1:
            for s in range(out.shape[0]):
                ...
                if ii in ids_first:
                    ...
                    ranks_later.append(rank_target + 1)
    ...
    sum_accuracy_later = np.sum(accuracies_later)

    current_accuracy_later = sum_accuracy_later / len(accuracies_later)

    MRR_later = np.sum([1 / r for r in ranks_later]) / len(ranks_later)

    print(len(accuracies), len(accuracies_later), len(ranks), len(ranks_later))

    print('First', int(sum_accuracy), round(current_accuracy, 5), 'MRR', round(MRR, 5))

    print('Later', int(sum_accuracy_later), round(current_accuracy_later, 5), 'MRR', round(MRR_later, 5))

    return current_accuracy, MRR, current_accuracy_later, MRR_later
...
if __name__ == '__main__':
    ...
    # from speaker copy dataset, where prev is nohs
    # these jsons come from compare_ref_hyp..TEST_BREAKDOWN
    # where we obtain detailed data from SpeakerDatasetCopy
    # regarding the actual prev and current etc.
    # examples come in the same order in the ListenerDataset
    # checked it via printing game and utt ids

    with open('speaker/ids_first.json', 'r') as f:
        ids_first = json.load(f)

    with open('speaker/ids_later.json', 'r') as f:
        ids_later = json.load(f)

    print(len(ids_first), len(ids_later))
    ...
    for hyps_file_path in all_hyps_files:
        ...
        for seed in [28]:
            ...
            with torch.no_grad():
                ...
                print('test')
                current_accuracy, MRR, current_accuracy_later, MRR_later = evaluate_trained_model(test_loader, testset, model, device, hyps, model_bert,
                                                               tokenizer, ids_first, ids_later)

                print('First Accuracy', current_accuracy, 'MRR', MRR, 'Later Accuracy', current_accuracy_later, 'MRR', MRR_later)
