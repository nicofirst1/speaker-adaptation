    ...
    for j in np.arange(params["num_iterations"]):
        ...
        if params["stepsize_search"] == "geometric_progression":
            ...
            perturbed, dist_post_update = binary_search_batch(
                sample, perturbed[None], model, params
            )
            ...
            if torch.sum(idx_perturbed) > 0:
                ...
                perturbed, dist_post_update = binary_search_batch(
                    sample, perturbeds[idx_perturbed], model, params
                )
        ...
        if verbose:
            print(
                "iteration: {:d}, {:s} distance {:.4E}".format(j + 1, constraint, dist)
            )
...
def initialize(model, sample, params):
    ...
    if params["target_image"] is None:
        ...
        while True:
            random_noise = (
                torch.FloatTensor(params["shape"])
                .uniform_(params["clip_min"], params["clip_max"])
                .to(params["device"])
            )
