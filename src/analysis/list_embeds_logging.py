...
from src.commons import get_listener_check, load_wandb_checkpoint, parse_args
from src.data.dataloaders import Vocab

global common_p
tsne_params = dict(
    perplexity=50, n_components=2, init="pca", n_iter=2500, random_state=23, verbose=1
)
...
def plot_embeds(main_df, unk_df, zero_df, domain_words):
    ...
    all_embeds = clustering(
        clustering_mode, clustering_type, all_embeds, main_df.shape[0]
    )
    ...
    # add marker patches
    patches.append(
        matplotlib.lines.Line2D([], [], marker=markers[0], label="main", markevery=0)
    )
    patches.append(
        matplotlib.lines.Line2D([], [], marker=markers[1], label="unk", markevery=0)
    )
    patches.append(
        matplotlib.lines.Line2D([], [], marker=markers[2], label="zero", markevery=0)
    )
...
def mask_oov_embeds_custom(
    ...
    replacement = (
        current_embeds[unk_i]
        if replace_token == "unk"
        else torch.zeros(current_embeds.shape[1])
    )
...
def main():
    ...
    list_full_embeddings = (
        list_checkpoint["model_state_dict"]["embeddings.weight"].cpu().numpy()
    )

    list_tokens = list(list_vocab.word2index.keys())

    with torch.no_grad():
        unk_embeddings = mask_oov_embeds_custom(
            copy.deepcopy(list_full_embeddings),
            list_vocab,
            domain,
            replace_token="unk",
            data_path=common_p.data_path,
        )
        zero_embeddings = mask_oov_embeds_custom(
            copy.deepcopy(list_full_embeddings),
            list_vocab,
            domain,
            replace_token="zero",
